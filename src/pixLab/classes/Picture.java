...
public class Picture extends SimplePicture 
{
  ...
  public void grayscale()
  {
	  ...
	  for (Pixel[] rowArray : pixels)
	  {
		  for (Pixel pixelObj : rowArray)
		  {
			 ...
			 int colorAvg = (pixelObj.getRed() + pixelObj.getBlue() + pixelObj.getGreen()) / 3;
  ...
  public void mirrorDiagonal()
  {
    ...
    Pixel rightPixel = null;
    int width = pixels[0].length;
    double slope = (double) pixels.length / width;
    for (int row = 0; row * slope < pixels.length; row++)
    {
      for (int col = 0; col < row; col++)
      {
        leftPixel = pixels[(int) (col * slope)][row];
        rightPixel = pixels[(int) (row * slope)][col];
        rightPixel.setColor(leftPixel.getColor());
      }
    } 
  }
